...
export default function LiveLog({ events = [] }) {
  ...
  const getEventColor = (event) => {
    if (event.status === 'job_completed') return 'text-green-600 dark:text-green-400';
    if (event.status === 'job_failed') return 'text-red-600 dark:text-red-400';
    if (event.status === 'job_started') return 'text-blue-600 dark:text-blue-400';
          ...
          {events.map((event, index) => (
            <div key={index} className="flex items-start gap-3">
              ...
              <span className={`flex-1 ${getEventColor(event)}`}>
                {event.message || event.stage || 'Processing...'}
