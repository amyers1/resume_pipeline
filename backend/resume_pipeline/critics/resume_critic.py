...
class ResumeCritic:
    ...
    def critique_and_refine(self, draft: str, jd: JDRequirements) -> tuple[str, dict]:
        """
        ...
        for iteration in range(1, self.config.max_critique_loops + 1):
            ...
            print(
                f"    Score: {critique.score:.3f} | "
                f"Coverage: {critique.jd_keyword_coverage:.3f} | "
                f"Length OK: {critique.length_ok}"
            )

            # Check if we meet threshold (relaxed criteria)
            # Use 'min_quality_score' from config but normalized to 0-1 if config uses 1-10
            threshold_score = self.config.critique_threshold
            if threshold_score > 1.0:
                threshold_score /= 10.0  # Normalize 1-10 scale to 0-1

            meets_threshold = (
                critique.score >= threshold_score
                and critique.jd_keyword_coverage
                >= (threshold_score - 0.2)  # Allow slightly lower coverage
    ...
    def _critique(self, resume: str, jd: JDRequirements) -> CritiqueResult:
        ...
        # Note: with_structured_output works for both OpenAI and Gemini (langchain-google-genai >= 1.0)
        chain = self.critic_prompt | self.llm.with_structured_output(CritiqueResult)

        critique = chain.invoke({"jd_json": jd.model_dump_json(), "resume": resume})
        return critique

    def _refine(self, resume: str, critique: CritiqueResult, jd: JDRequirements) -> str:
